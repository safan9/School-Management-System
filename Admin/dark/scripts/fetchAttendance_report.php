<?php
	...
	if($month_id != '0'){
		...
		if(mysqli_num_rows($result) > 0){
			...
			$sql = "
				SELECT attendance_mst.*, division_mst.*, month_mst.month_name, student_mst.*, academin_year_mst.*,staff_mst.* 
				FROM attendance_mst
			...
			if(mysqli_num_rows($res) > 0){
				...
				$output .= "
					...
					<a class='btn btn-primary' href='scripts/download_attendance_report.php?month=$month_id&year=$academic_year_id&div_id=$div_id&name=$staff_name'>Print Preview</a>
				";
			}else{
